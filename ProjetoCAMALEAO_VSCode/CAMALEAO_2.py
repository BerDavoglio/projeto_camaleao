...
for j in range(0, altura + passo, passo):
    for i in range (0, largura + passo, passo):
        ...
        cart = Carteira(i, j)
        if cart.d_tot():
            if len(l_carteiras) > 0:
                for b in l_carteiras:
                    if (i, j) != b:
                        if d_carteira(i, j, b[0], b[1]) >= (raio_carteiras * 2):
                            l_carteiras_atualizada.append((i, j))
            else:
                l_carteiras_atualizada.append((i, j))
...
for ind, qq in enumerate(l_carteiras_atualizada):
